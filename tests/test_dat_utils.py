from utils.dat_utils import parse_dat, parse_custom_dat
...
@pytest.mark.parametrize("dat_path,expected_sha1,expected_md5,expected_crc,expected_filename", [
    *[(Path(dat),
    ...
    *[(Path(dat),
        "d2a7729c02f5b409ea5032e0a1dd6416816eeb10",  # 007 - Quantum of Solace (USA) (En,Fr)
        "23d8012fa0374a06214072156eb4fc28",
        "67f2b05f",
        "007 - Quantum of Solace (USA) (En,Fr).nds",
    ) for dat in no_intro_nds_encrypted_dats],
    *[(Path(dat),
        "e4d14529bf26eb8af1763a399ee6e7b176b87840",  # Learn with Pokemon - Typing Adventure (Europe)
        "8c39b2846857f4ca276a2a167964e1dd",
        "28dd5577",
        "Learn with Pokemon - Typing Adventure (Europe).nds",
    ) for dat in no_intro_nds_encrypted_dats],
])
def test_parse_dat_no_intro_xml(dat_path, expected_sha1, expected_md5, expected_crc, expected_filename):
...
def test_parse_dat_clrmamepro(dat_path, expected_sha1, expected_md5, expected_crc, expected_filename):
